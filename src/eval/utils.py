...
def load_hooked_lm_and_tokenizer(
    ...
    """
    Loads a Hooked* model and (optionally) overlays PEFT adapters.
    If peft_name_or_path includes items that are NOT PEFT dirs (no adapter_config.json),
    they are silently ignored to support passing full checkpoints by mistake.
    """
    ...
    # ---- tolerant PEFT application ----
    def _is_peft_dir(path):
        if not path or not os.path.isdir(path):
            return False
        # Common PEFT fingerprints
        if os.path.exists(os.path.join(path, "adapter_config.json")):
            return True
        # some repos distribute adapter weights without config name conventions; be conservative
        candidates = glob.glob(os.path.join(path, "*adapter*.safetensors"))
    ...
    if peft_name_or_path:
        ...
        for path in peft_name_or_path:
            if not _is_peft_dir(path):
                print(f"[load_hooked] Skip non-PEFT path: {path}")
                continue
            try:
                peft_config = json.load(open(os.path.join(path, "adapter_config.json")))
                setattr(hook_model, "peft_type", peft_config.get("peft_type", "UNKNOWN"))
                print(f"Load {peft_config.get('peft_type', 'PEFT')} from {path}.")
                if peft_config.get("peft_type") == "PROMPT_TUNING":
                    hook_model.set_prompt_embeddings(path)
                elif peft_config.get("peft_type") == "IA3":
                    hook_model.set_ia3(path)
                elif peft_config.get("peft_type") == "LORA":
                    hook_model.set_lora(path, peft_config)
                else:
                    # fallback: try LORA loader if weights look like LoRA but no type field
                    if glob.glob(os.path.join(path, "*adapter*.safetensors")):
                        hook_model.set_lora(path, peft_config)
            except Exception as e:
                print(f"[load_hooked] Failed to apply PEFT at {path}: {e}. Skipping.")
